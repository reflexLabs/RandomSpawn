...
public class RandomCommands implements CommandExecutor
{
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        if (args.length <= 0) {
            ...
            } else if(args[0].equalsIgnoreCase("teleport") || args[0].equalsIgnoreCase("force")) {
            	if(sender.hasPermission("randomspawn.teleport") || sender.hasPermission("randomspawn.force") || sender.hasPermission("randomspawn.*") || sender.isOp()) {
            		try {
            			String pointID;
            			String target;
